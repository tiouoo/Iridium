...
/// <summary>
/// Builds launch arguments from a resolved <see cref="MinecraftContext"/>. The parser is
/// format-agnostic: directory mapping comes from <c>context.Layout</c>, and any
/// format-specific arguments are contributed through the context's format provider.
/// </summary>
public sealed partial class ArgumentParser : IArgumentParser {
    ...
    public LaunchArguments Build(MinecraftContext context, LaunchConfig config) {
        ...
        var paths = LaunchDirectories.Resolve(layout, entry, config);
        ...
        context.Provider?.ConfigureArguments(builder, context);
        jvmArguments.AddRange(builder.JvmArguments);
        gameArguments.AddRange(builder.GameArguments);

        var mainClass = builder.MainClass is { Length: > 0 }
            ? builder.MainClass
            : string.IsNullOrWhiteSpace(entry.MainClass)
                ? DefaultMainClass
                : entry.MainClass;
