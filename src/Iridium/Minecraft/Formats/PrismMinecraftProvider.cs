...
/// </summary>
public sealed class PrismMinecraftProvider : IFormatProvider {
    public string Id => "Prism";

    public int Priority => 90;

    public bool CanResolve(DirectoryInfo root) {
        var instancesDir = Path.Combine(root.FullName, "instances");
        if (!Directory.Exists(instancesDir))
            return false;

        // Require at least one instance carrying a Prism manifest so we don't shadow
        // other launchers that also use an instances/ folder (e.g. Portal MC).
        return Directory.EnumerateDirectories(instancesDir)
            .Any(dir => File.Exists(Path.Combine(dir, "mmc-pack.json")) ||
    ...
    public async ValueTask<IReadOnlyList<MinecraftContext>> GetMinecraftsAsync(DirectoryInfo root, CancellationToken ct = default) {
        var instancesDir = new DirectoryInfo(Path.Combine(root.FullName, "instances"));
    ...
    private async Task<MinecraftEntry?> ParseAsync(DirectoryInfo dir, string launcherRoot, CancellationToken ct) {
        ...
        var json = await File.ReadAllTextAsync(packPath, ct);
        ...
        return await MergeComponentsAsync(entry, componentList, dir, launcherRoot, ct);
    }

    private static async Task<MinecraftEntry> MergeComponentsAsync(
        ...
        try {
            foreach (var (uid, version) in components) {
                var metaPath = Path.Combine(launcherRoot, "meta", uid, $"{version}.json");
                if (!File.Exists(metaPath))
                    continue;

                docs.Add((JsonDocument.Parse(await File.ReadAllTextAsync(metaPath, ct)), uid));
            }
            ...
            if (mainClass == "net.minecraft.launchwrapper.Launch") {
                var detectedTweakers = ReadTweakClasses(loaderLibraries, Path.Combine(launcherRoot, "libraries"));
